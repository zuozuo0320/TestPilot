...
- 标签管理 + 用例评审 + AI 门禁 Action Items
- 需求/脚本管理 + 执行触发 + 缺陷管理 + 审计日志
- **测试智编**（AI 脚本生成 + 录制增强 + Playwright 验证 + 多项目工作区）
- **需求智生**（需求文档解析 + AI 用例生成 + Skill 路由 + 队列/本地执行降级）
- AI 模型配置（连接测试、模型列表、唯一启用、同步 Executor）
...
如需更新历史需求/架构/OpenAPI 文档，请优先对齐：

- `docs/测试管理平台-当前实现业务逻辑总览-20260605.md`
- 后端路由事实源：`internal/api/router.go`
- 后端业务规则事实源：`internal/service/`
- Executor endpoint 事实源：`executor/main.py`
