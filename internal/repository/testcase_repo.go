...
func (r *testCaseRepo) ListPaged(ctx context.Context, projectID uint, f TestCaseFilter) ([]TestCaseListItem, int64, error) {
	...
	reviewProjectionSelect := strings.Join([]string{
		"test_cases.id",
		...
		`CASE WHEN EXISTS (
			...
			WHERE cri.test_case_id = test_cases.id
			  ...
			  AND cri.review_status IN ('pending', 'reviewing')
			  AND cr.status IN ('not_started', 'in_progress')
		) THEN TRUE ELSE FALSE END AS in_review`,
		`COALESCE((
			SELECT cr.id
			FROM case_review_items cri
			JOIN case_reviews cr ON cr.id = cri.review_id AND cr.project_id = test_cases.project_id
			WHERE cri.test_case_id = test_cases.id
			  ...
			  AND cr.status IN ('not_started', 'in_progress')
			ORDER BY
			  CASE WHEN cr.status = 'in_progress' THEN 0 ELSE 1 END,
			  cri.updated_at DESC,
			  cr.updated_at DESC,
			  cr.id DESC
			LIMIT 1
		), 0) AS current_review_id`,
		`COALESCE((
			SELECT cr.name
			FROM case_review_items cri
			JOIN case_reviews cr ON cr.id = cri.review_id AND cr.project_id = test_cases.project_id
			...
			ORDER BY
			  ...
			  cr.id DESC
			LIMIT 1
		), '') AS current_review_name`,
		`COALESCE((
			SELECT COUNT(DISTINCT cri.review_id)
			FROM case_review_items cri
			WHERE cri.test_case_id = test_cases.id
			  AND cri.project_id = test_cases.project_id
		), 0) AS related_review_count`,
	}, ", ")
	err := baseQuery.
		Select(reviewProjectionSelect).
