...
var (
	...
	ErrUserDisabled         = ErrForbidden("USER_DISABLED", "账号已被禁用")
	...
	ErrProjectArchived      = ErrConflict("PROJECT_ARCHIVED", "项目已归档，不可操作")
	ErrProjectNotArchived   = ErrConflict("PROJECT_NOT_ARCHIVED", "项目未归档，无需恢复")
	ErrProjectNotEmpty      = ErrConflict("PROJECT_NOT_EMPTY", "项目下仍有数据，无法删除")
	ErrSeedProjectProtected = ErrConflict("SEED_PROJECT_PROTECTED", "种子项目「快速开始」不允许删除或归档")
	ErrPasswordTooWeak      = ErrBadRequest("PASSWORD_TOO_WEAK", "密码至少8位，须包含大写字母、小写字母和数字")
	ErrOldPasswordWrong     = ErrBadRequest("OLD_PASSWORD_WRONG", "旧密码不正确")
	ErrEmailImmutable       = ErrBadRequest("EMAIL_IMMUTABLE", "邮箱不可修改")
)
