...
export class NavigationPage {
  ...
  /**
   * 优先在左侧菜单容器里获取可见的菜单触发节点，避免误点到页面内容区的同名标题。
   * @param text 菜单文字
   */
  async getVisibleMenuTrigger(text: string): Promise<Locator> {
    const candidateGroups = [
      this.page.locator('.el-submenu__title, .el-menu-item').filter({ hasText: text }),
      this.page.locator('.el-menu span').filter({ hasText: text }),
    ...
    return this.getVisibleExactText(text);
  }

  /**
   * 通过菜单名称导航到指定页面。
   * @param menuName 菜单项文字
   */
  async goToMenu(menuName: string): Promise<void> {
    const menuItem = await this.getVisibleMenuTrigger(menuName);
  ...
  /**
   * 通过多级菜单路径逐级导航。
   * @param menuPath 菜单路径数组，如 ['任务管理', '资产探知']
   */
