...
class GenerateRequest(BaseModel):
    ...
    extra_prompt: str = ""
    existing_tags: list[str] = []   # 项目现有标签名，约束 AI 只能从中选取 tags_suggested
    project_context: str = ""       # 项目业务背景（名称/描述）
...
def _build_user_prompt_for_skill(skill: SkillItem, req: GenerateRequest, max_cases: int) -> str:
    ...
    prompt = prompt.replace("{{existing_tags}}", ", ".join(req.existing_tags) if req.existing_tags else "无")
    prompt = prompt.replace("{{project_context}}", req.project_context or "无")
...
8. 如果需求涉及异常处理、超时、重试、降级 → 选 exception_resilience
9. 如果需求涉及多个独立输入参数的组合（如商品规格、筛选条件）→ 选 pairwise_combination
10. 如果需求涉及角色权限、操作授权 → 选 rbac_permission
11. 如果需求涉及数据增删改查（CRUD）操作 → 选 data_integrity_crud
12. 如果需求涉及界面交互、表单操作、页面状态、提示文案、布局适配 → 选 usability_ui
13. 如果需求涉及输入校验、字段格式约束、非法输入处理 → 选 negative_equivalence
14. 如果需求涉及版本升级、数据迁移、表结构变更、历史数据兼容 → 选 data_migration
