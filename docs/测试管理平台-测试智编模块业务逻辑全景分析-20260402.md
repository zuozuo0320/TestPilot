...
stateDiagram-v2
    ...
    GENERATE_FAILED --> DISCARDED: 废弃
    GENERATE_SUCCESS --> DISCARDED: 废弃
    PENDING_REVALIDATE --> DISCARDED: 废弃
    PENDING_CONFIRM --> DISCARDED: 废弃
    CONFIRMED --> DISCARDED: 废弃
    MANUAL_REVIEW --> DISCARDED: 废弃
...
| POST | `/tasks/batch-discard` | `batchDiscardTasks` | 批量废弃任务（支持跨页全选、按筛选结果全选） |
| DELETE | `/tasks/:taskID` | `deleteTask` | 物理删除（仅已废弃） |
| POST | `/tasks/batch-delete` | `batchDeleteTasks` | 批量删除任务（仅删除已废弃任务，支持跨页全选、按筛选结果全选） |
...
| | `taskSelection` | 任务列表批量勾选状态（`selectionMode / selectedIds / excludedIds / filterSnapshot / expectedTotal`） |
| **写** | `createTask` | 创建任务 → 刷新列表 |
| | `executeTask` | 触发执行 → 刷新详情 |
| | `updateScript` | 编辑脚本 → 刷新脚本+版本+详情 |
| | `runValidation` | 触发验证 → 刷新验证+详情 |
| | `discardTask` | 废弃任务 → 刷新列表 |
| | `batchDiscardTasks` | 批量废弃任务 → 刷新列表并清空勾选 |
| | `deleteDiscardedTask` | 物理删除 → 刷新列表 |
| | `batchDeleteTasks` | 批量删除任务 → 刷新列表并清空勾选 |

### 9.4 任务列表批量治理规则（2026-04-03 确认版）

#### 9.4.1 选择模型
- 任务列表必须支持 3 种选择方式：当前页勾选、跨页累计勾选、按当前筛选结果全选。
- 前端不能只保存 `selectedIds`，必须统一维护 `selectionMode + selectedIds + excludedIds + filterSnapshot + expectedTotal`。
- `selectionMode = IDS` 表示显式勾选任务集合；`selectionMode = FILTER_ALL` 表示“当前筛选命中的所有任务均默认选中”，此时通过 `excludedIds` 记录反选项。
- 翻页后必须保留勾选状态；一旦 `project_id / keyword / task_status` 任一筛选条件发生变化，必须清空当前选择并重置为第一页。

#### 9.4.2 跨页全选与按筛选结果全选
- “跨页全选”本质上是用户在不同分页下持续维护同一批 `selectedIds`。
- “按筛选结果全选”必须严格基于服务端真实筛选结果，不能以前端当前页数据推断。
- 进入 `FILTER_ALL` 模式后，界面需明确展示“已选择当前筛选结果的 N 条任务”，并允许用户在单条任务上取消勾选形成 `excludedIds`。
- 当前模块明确支持“跨页全选”和“按筛选结果全选”，不再限制为“仅当前页勾选”。

#### 9.4.3 批量废弃规则
- 批量废弃为正式能力，不再仅支持单条废弃。
- 批量废弃必须填写统一废弃原因，并写入每条任务的 `discard_reason`。
- 允许批量废弃的任务状态固定为：`DRAFT / PENDING_EXECUTE / GENERATE_FAILED / GENERATE_SUCCESS / PENDING_CONFIRM / PENDING_REVALIDATE / CONFIRMED / MANUAL_REVIEW`。
- `RUNNING` 任务禁止批量废弃；`DISCARDED` 任务禁止重复废弃。
- 批量废弃后保留历史脚本版本、验证记录、操作日志与录制数据，不做物理删除。

#### 9.4.4 批量删除规则
- 批量删除为正式能力，但只允许删除 `DISCARDED` 状态任务。
- 非 `DISCARDED` 任务即使被选中，也只能计入跳过/失败统计，服务端不得放宽规则。
- 批量删除沿用单条删除的级联清理策略：任务、关联用例关系、脚本版本、验证记录、轨迹、证据、录制会话、操作日志一并删除。

#### 9.4.5 权限与执行反馈
- 仅 `admin / manager` 可执行批量废弃与批量删除。
- 批量接口必须支持部分成功返回，返回结果至少包含：`matched / success / skipped / failed`。
- `skipped` 与 `failed` 需按原因聚合统计，例如“运行中不可废弃”“未废弃不可删除”“任务不存在”。
- 前端批量操作完成后必须刷新列表、清空勾选状态，并用 toast/弹窗反馈操作摘要。
